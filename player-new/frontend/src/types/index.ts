...
export interface Track { id: string; title: string; artist: string; file_url: string; cover_url?: string; duration: number; file_size: number; mime_type: string; fingerprint_hash?: string; waveform?: number[]; loudness?: number; created_at: string }
...
export interface RecognitionResult { track_id?: string; title?: string; artist?: string; cover_url?: string; confidence?: number; status: 'listening'|'processing'|'found'|'not_found' }
